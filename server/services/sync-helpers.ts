...
export async function findExistingRoundSnapshot(tx: {
  query: {
    standingsSnapshots: {
      findFirst: DbTransaction['query']['standingsSnapshots']['findFirst']
    }
  }
}, roundNumber: number) {
  return tx.query.standingsSnapshots.findFirst({
    where: and(eq(standingsSnapshots.season, APP_SEASON), eq(standingsSnapshots.roundNumber, roundNumber)),
  })
}
...
export async function insertSnapshotBundle(
  tx: DbTransaction,
  live: LiveBundle,
  leaderboard: ComputedParticipantScore[],
) {
